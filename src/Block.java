...
public class Block {
    ...
    private int iDifficulty = 4; // Mining seconds in testing 5: 6,10,15,17,20,32 | testing 6: 12,289,218
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
        ...
        ArrayList<MerkleNode> nodeLst = new ArrayList<>();
        int lstLength = lstItems.size();

        // while ((int) Math.ceil((Math.log(lstLength) / Math.log(2))) == (int) (Math.floor((Math.log(lstLength) / Math.log(2))))) {

            for (int x=0; x < lstItems.size(); x++) {
                MerkleNode oNode = new MerkleNode();
                oNode.sHash = BlockchainUtil.generateHash(lstItems.get(x));
                nodeLst.add(oNode);
            }
            for (int x=0; x < nodeLst.size(); x+=2) {
                MerkleNode oNode = new MerkleNode();
                try{
                    populateMerkleNode(oNode, nodeLst.get(x), nodeLst.get(x+1));
                    nodeLst.add(oNode);
                } catch(IndexOutOfBoundsException e){
                    break;
                }
            }
            return nodeLst.get(nodeLst.size()-1).sHash;
    //    }
       // System.out.println("Please enter 2, 4, 8, 16... items.");
       // return null;
    }
